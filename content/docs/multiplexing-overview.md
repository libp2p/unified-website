...
description = "Stream Multiplexing is a way of sending multiple streams of data over one communication link. It combines multiple signals into one unified signal so it can be transported 'over the wires', then it is demultiplexed so it can be output and used by separate applications."
weight = 40
aliases = ["/concepts/stream-multiplexing", "/concepts/multiplex", "/guides/multiplexing-overview/"]
...
Currently, libp2p supports two stream muxers, [mplex](/docs/mplex/)
and [yamux](/docs/yamux/). However, many of the
[transport protocols](/docs/transports-overview/) available in the libp2p stack
come with native streams, such as [QUIC](/docs/quic/),
[WebTransport](/docs/webtransport/), and
[WebRTC](/docs/webrtc/), and in these cases, libp2p
