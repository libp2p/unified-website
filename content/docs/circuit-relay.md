...
weight = 52
aliases = ["/concepts/circuit-relay", "/concepts/nat/circuit-relay", "/guides/circuit-relay/"]
...
Circuit relay is a [transport protocol](/docs/transports-overview/) that routes traffic between two peers over a third-party "relay" peer.

In many cases, peers will be unable to [traverse their NAT and/or firewall](/docs/nat-overview/) in a way that makes them publicly accessible. Or they may not share common [transport protocols](/docs/transports-overview/) that would allow them to communicate directly.
...
A relay circuit is identified using a multiaddr that includes the [Peer ID](/docs/peers/#peer-id) of the peer whose traffic is being relayed (the listening peer or "relay target").
...
1. Node `A` is behind a NAT and/or firewall, e.g. detected via the [AutoNAT service](/docs/autonat/).
