...
weight = 13
aliases = ["/concepts/protocols", "/concepts/fundamentals/protocols", "/guides/protocols/"]
...
using the core libp2p abstractions like transport, [peer identity](/docs/peers/#peer-id), [addressing](/docs/addressing/), and so on.
...
developer-friendly and enables [easier matching by version](#using-a-match-function).
{% end %}
...
switch (aka "swarm"), or a higher level interface such as [go's Host interface](https://github.com/libp2p/go-libp2p/blob/master/core/host/host.go).
...
Behind the scenes, libp2p will also ensure that the stream is secure and efficiently
multiplexed. This is transparent to the protocol handler, which reads and writes
...
information about themselves, such as their public key, which is used to derive their [PeerId](/docs/peers/).

Importantly, the `Identify` message includes an `observedAddr` field that contains the multiaddr that
...
libp2p uses the DHT as the foundation of its routing functionality. To learn more about DHT and the Kademlia algorithm,
check out the [Distributed Hash Tables guide][dht] on the IPFS documentation site. In addition, check out the [libp2p implementations page](https://github.com/libp2p/libp2p#implementations) for updates on all the kad-libp2p implementations.
...
directly. See the [Circuit Relay documentation](/docs/circuit-relay/) for more information on working with relays, including notes on relay
