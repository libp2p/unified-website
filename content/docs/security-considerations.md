...
weight = 15
aliases = ["/concepts/security-considerations", "/concepts/security/security-considerations", "/guides/security-considerations/"]
...
Every libp2p peer is uniquely identified by their [Peer ID](/docs/peers/#peer-id), which
...
Your [protocol handler](/docs/protocols/#handler-functions) could then
...
The Kad-DHT protocol is a distributed hash table that provides a
...
[Sybil attacks](https://en.wikipedia.org/wiki/Sybil_attack), in which one operator spins up a large number
...
and signing IDs from a central trusted authority. These mitigations are outside
...
parallel, inspired by the [S/Kademlia paper](https://telematics.tm.kit.edu/publications/Files/267/SKademlia_2007.pdf). This will
greatly increase the chances of finding "honest" nodes, even if some nodes are
returning dishonest routing information.

### Publish / Subscribe

libp2p's [publish/subscribe protocol](/docs/pubsub/) allows a peer
