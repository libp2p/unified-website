...
weight = 22
aliases = ["/concepts/transports/quic", "/guides/quic/"]
...
In contrast, HTTP/3 runs over QUIC, where QUIC is similar to
...
streams. Since QUIC packets can be decrypted even when they're received out of order, this solves the problem of HoL blocking that multiplexers applied on top of a TCP connection suffer from: If a packet that contains
