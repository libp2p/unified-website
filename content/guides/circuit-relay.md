...
weight = 52
aliases = ["/concepts/circuit-relay", "/concepts/nat/circuit-relay"]
...
Circuit relay is a [transport protocol](/guides/transports-overview/) that routes traffic between two peers over a third-party "relay" peer.

In many cases, peers will be unable to [traverse their NAT and/or firewall](/guides/nat-overview/) in a way that makes them publicly accessible. Or they may not share common [transport protocols](/guides/transports-overview/) that would allow them to communicate directly.
...
A relay circuit is identified using a multiaddr that includes the [Peer ID](/guides/peers/#peer-id) of the peer whose traffic is being relayed (the listening peer or "relay target").
...
The address above is interesting, because it doesn't include any [transport](/guides/transports-overview/) addresses for either the peer we want to contact (`QmAlice`) or for the relay peer that will convey the traffic. Without that information, the only chance a peer has of dialing me is to discover a relay and hope they have a connection to me.
...
1. Node `A` is behind a NAT and/or firewall, e.g. detected via the [AutoNAT service](/guides/autonat/).
